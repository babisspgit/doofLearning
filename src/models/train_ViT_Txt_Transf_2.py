...
def main(config=None):
    ...
    with wandb.init(config=config):
        ...
        save_path = Path("/zhome/39/c/174709/git/doofLearning/models")
        ...
        # ViT
        image_dims = config.vit_image_dims
        patch_dims = config.vit_patch_dims
        num_heads_vit = config.vit_num_heads
        num_blocks_vit = config.vit_num_blocks

        # Text Transformer
        num_heads_text = config.text_num_heads
        num_blocks_text = config.text_num_blocks
        ...
        # For saving time, take part of training dataset
        indices = np.arange(len(train_dataset) // 3)
