...
class ClipSymmetricLoss(nn.Module):
    ...
    def forward(self, img_emb, text_emb):
        ...
        loss_i = self.cross_entropy_loss(logits, labels, axis=0) # for image to text (rows)
        loss_t = self.cross_entropy_loss(logits, labels, axis=1) # for text to image (columns)
